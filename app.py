...
# تصميم داكن متكامل لحل مشكلة الخطوط البيضاء تماماً
st.markdown("""
    <style>
...
# بيانات الأسهم مع اللوجو التعريفي
DEFAULT_STOCKS = [
    {"icon": "⚙️", "name": "العربية للصناعات الهندسية", "ticker": "EEII", "qty": 24372, "avg": 2.2904, "price": 2.35},
    {"icon": "🌾", "name": "نهر الخير للتنمية والاستثمار", "ticker": "KRDI", "qty": 123690, "avg": 0.4159, "price": 0.449},
    {"icon": "🏢", "name": "القاهرة للإسكان والتعمير", "ticker": "ELKA", "qty": 21990, "avg": 1.7544, "price": 1.87},
    {"icon": "🏺", "name": "سيراميكا ريماس", "ticker": "CERA", "qty": 22100, "avg": 1.3159, "price": 1.50},
    {"icon": "🏗️", "name": "المصريين للإسكان والتنمية", "ticker": "EHDR", "qty": 9793, "avg": 2.6623, "price": 2.88},
    {"icon": "💎", "name": "العز سيراميك (الجوهرة)", "ticker": "ECAP", "qty": 365, "avg": 34.4619, "price": 33.62},
    {"icon": "🔩", "name": "مصر الوطنية للصلب (عتاقة)", "ticker": "ATQA", "qty": 592, "avg": 12.6712, "price": 12.17},
    {"icon": "🛢️", "name": "أموك للزيوت المعدنية", "ticker": "AMOC", "qty": 449, "avg": 7.9226, "price": 13.50},
]
...
st.markdown(f"""
<div style="background: linear-gradient(135deg, #1e293b, #0f172a); border: 1px solid #334155; border-radius: 14px; padding: 18px; margin-bottom: 18px; text-align: center;">
    ...
    for _, row in df.iterrows():
        ...
        <div class="card">
            ...
            <div class="card-text">
                ...
                <div>متوسط الشراء: <b style="color: #f8fafc;">{row['avg']:.4f} ج.م</b> | السعر الحالي: <b style="color: #60a5fa;">{row['price']:.2f} ج.م</b></div>
...
with tab2:
    st.markdown("<div style='color: #94a3b8; font-size: 13px; margin-bottom: 12px;'>مستويات الدعم والمقاومة والاتجاه الفني لكل سهم:</div>", unsafe_allow_html=True)
    for _, row in df.iterrows():
        p = row["price"]
        sup1 = round(p * 0.96, 2)
        res1 = round(p * 1.05, 2)
...
with tab3:
    st.markdown("<div style='color: #f8fafc; font-size: 16px; font-weight: bold; margin-bottom: 8px;'>تسجيل حركة كاش أو مصروف</div>", unsafe_allow_html=True)
    with st.form("cash_form"):
        ...
        desc = st.text_input("البيان (مثال: بنزين، خروج، فواتير، أرباح تداول):")
        if st.form_submit_button("حفظ الحركة", use_container_width=True):
            ...
            st.success("تم الحفظ بنجاح وتحديث الرصيد!")
            st.rerun()

    if st.session_state.expenses:
        st.write("---")
        st.markdown("<div style='color: #cbd5e1; font-weight: bold; margin-bottom: 8px;'>آخر المعاملات المسجلة:</div>", unsafe_allow_html=True)
        for exp in reversed(st.session_state.expenses[-5:]):
            st.markdown(f"<div style='color: #94a3b8; font-size: 12px; margin-bottom: 4px;'>• {exp['التاريخ']} | {exp['النوع']}: <b style='color: #f8fafc;'>{exp['المبلغ']} ج.م</b> ({exp['البيان']})</div>", unsafe_allow_html=True)
